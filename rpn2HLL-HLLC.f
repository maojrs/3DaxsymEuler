...
c     # using HLL - HLLC hybrid approximate Riemann solver
c
...
!          ! Compute the speed of left and right HLLC wave
         ...
         ! Calculate fluxes
         fluxl(1,i) = rho_l*ul
         fluxl(mu,i) = rho_l*ul**2 + pl
         fluxl(mv,i) = rho_l*ul*vl
         fluxl(4,i) = ul*(qr(4,i-1) + pl)
         fluxr(1,i) = rho_r*ur
         fluxr(mu,i) = rho_r*ur**2 + pr
         fluxr(mv,i) = rho_r*ur*vr
         fluxr(4,i) = ur*(ql(4,i) + pr)

         ! Calculate ql* and qr* HLL middle states (without pressure term)
         do j=1,meqn
             qm(j,i) = (Sr*ql(j,i) - Sl*qr(j,i-1) + 
     & fluxl(j,i) - fluxr(j,i))/(Sr - Sl)
         end do
         
         if (gammal .ne. 1.4 .or. gammar .ne. 1.4) then
            ! Do HLLC metod at interface
            
            ! Compute HLLC middle speed state (see research notebook)
            Sm = pr - pl + rho_r*ur*(ur - Sr) - rho_l*ul*(ul - Sl) 
            Sm = Sm/(rho_r*(ur - Sr) - rho_l*(ul - Sl))
            s(2,i) = 0.d0*Sm ! Make zero to force fixed contact disc.
